document.addEventListener("DOMContentLoaded", () => {
    ...
    let fudgeTimeIntervalId = null; // Interval for fudge time logic
    ...
    function setupTimerControls(timerComponent, initialTime) {
      ...
      function startTimer() {
        ...
        startPauseBtn.textContent = "⏸️";
        stopFudgeTime(); // Stop fudge time if a timer starts
      }
  
      function startFudgeTime() {
        if (!fudgeTimeIntervalId) {
          let fudgeTimeSeconds = 0;
          fudgeTimeIntervalId = setInterval(() => {
            fudgeTimeSeconds++;
            const minutes = Math.floor(fudgeTimeSeconds / 60);
            const seconds = fudgeTimeSeconds % 60;
            timerValue.value = `${minutes}:${seconds.toString().padStart(2, "0")}`;
          }, 1000);
        }
      }
  
      function stopFudgeTime() {
        clearInterval(fudgeTimeIntervalId);
        fudgeTimeIntervalId = null;
      }
      ...
      removeBtn.addEventListener("click", () => {
        clearInterval(intervalId);
        clearInterval(fudgeTimeIntervalId); // Stop fudge time if timer is removed
      ...
      // Start fudge time if no timer is running
