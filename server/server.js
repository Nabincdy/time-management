        ...
        try {
            ...
            switch (message.cmd) {//FLAG: Nabin bind on client side (created 9:50am mst Feb 11 2025)
                ...
                case 'addVote':
                    ...
                    let voteUpdateMessage = {
                        cmd: 'updateVote',
                        msg: {
                            ELMxID: addVoteELMxID,
                            participantID: addVoteParticipantID,
                            voteData: participantInfo.participantCurrentVoting
                        }
                    };
            
                    // Broadcast the updated vote to everyone except the sender
                    clients.forEach(client => {
                        if (client !== ws && client.readyState === WebSocket.OPEN) {
                            client.send(JSON.stringify(voteUpdateMessage));
                        }
                    });
                    break;
